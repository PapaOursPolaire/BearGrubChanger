...
# Fonction pour changer d'environnement de bureau
function changer_environnement_bureau() {
    ...
    # Environnements disponibles avec leurs paquets
    declare -A environnements=(
        ["1"]="plasma-desktop:KDE Plasma"
        ["2"]="gnome:GNOME"
        ["3"]="xfce4:XFCE"
        ["4"]="cinnamon:Cinnamon"
        ["5"]="mate-desktop:MATE"
        ["6"]="lxqt:LXQt"
        ["7"]="lxde:LXDE"
        ["8"]="budgie-desktop:Budgie"
        ["9"]="deepin-desktop:Deepin"
        ["10"]="enlightenment:Enlightenment"
        ["11"]="i3:i3 WM"
        ["12"]="sway:Sway (Wayland)"
        ["13"]="openbox:Openbox"
        ["14"]="fluxbox:Fluxbox"
    )
    ...
    echo "============================"
    for key in "${!environnements[@]}"; do
        IFS=':' read -r paquet nom <<< "${environnements[$key]}"
        echo "$key. $nom ($paquet)"
    done
    ...
    if pgrep -x "plasmashell" >/dev/null; then
        ...
        echo "✓ Cinnamon détecté"
        env_actuel="cinnamon"
    else
        echo "ℹ Environnement non spécifiquement détecté"
        env_actuel="inconnu"
    fi
    
    # Choix de l'utilisateur
    read -p "Choisissez un environnement [1-14]: " choix_env
    
    if [ -z "${environnements[$choix_env]}" ]; then
        echo "Choix invalide."
        return 1
    fi
    
    IFS=':' read -r paquet_env nom_env <<< "${environnements[$choix_env]}"
    
    echo -e "\nVous avez choisi: $nom_env"
    
    # Sous-options
    echo -e "\nOPTIONS D'INSTALLATION:"
    echo "1. Remplacer l'environnement actuel"
    echo "2. Ajouter en multi-environnement (choix au login)"
    echo "3. Installer en mode minimal"
    echo "4. Annuler"
    
    read -p "Votre choix [1-4]: " mode_installation
    
    case "$mode_installation" in
        1)
            echo -e "\nREMPLACEMENT DE L'ENVIRONNEMENT ACTUEL"
            echo "========================================"
            echo "Attention: Cette option désinstallera l'environnement actuel"
            echo "et le remplacera par $nom_env"
            ...
            # Désinstaller l'environnement actuel si détecté
            if [ "$env_actuel" != "inconnu" ]; then
                echo "Désinstallation de l'environnement actuel..."
                desinstaller_environnement_actuel "$env_actuel"
            fi
            
            # Installer le nouvel environnement
            installer_environnement "$paquet_env" "$nom_env"
            
            # Configurer comme environnement par défaut
            configurer_environnement_par_defaut "$paquet_env"
            
            echo -e "\n✅ REMPLACEMENT TERMINÉ !"
            echo "Redémarrez pour utiliser $nom_env"
            ;;
            
        2)
            echo -e "\nAJOUT EN MULTI-ENVIRONNEMENT"
            echo "============================"
            echo "Installation de $nom_env en parallèle de l'environnement actuel"
            echo "Vous pourrez choisir au moment du login"
            
            # Installer l'environnement
            installer_environnement "$paquet_env" "$nom_env"
            
            echo -e "\n✅ ENVIRONNEMENT AJOUTÉ !"
            echo "$nom_env est maintenant disponible au login"
            ;;
            
        3)
            echo -e "\nINSTALLATION MODE MINIMAL"
            echo "=========================="
            echo "Installation de la version minimale de $nom_env"
            
            # Déterminer le paquet minimal
            case "$paquet_env" in
                "plasma-desktop") paquet_minimal="plasma-desktop" ;;
                "gnome") paquet_minimal="gnome-core" ;;
                "xfce4") paquet_minimal="xfce4" ;;
                "cinnamon") paquet_minimal="cinnamon-core" ;;
                "mate-desktop") paquet_minimal="mate-core" ;;
                *) paquet_minimal="$paquet_env" ;;
            esac
            
            installer_environnement "$paquet_minimal" "$nom_env (minimal)"
            
            echo -e "\n✅ VERSION MINIMALE INSTALLÉE !"
            ;;
    ...
    echo -e "\n📋 RECOMMANDATIONS:"
    echo "-------------------"
    echo "1. Redémarrez votre session pour voir les changements"
    echo "2. Au login, sélectionnez $nom_env dans le menu des environnements"
    echo "3. Personnalisez avec les options de BearGrubChanger"
    
    read -p "Redémarrer maintenant ? (y/N): " redemarrer
    if [[ "$redemarrer" =~ ^[Yy]$ ]]; then
        echo "Redémarrage dans 5 secondes..."
        sleep 5
        sudo reboot
    fi
...
function detecter_distribution() {
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        echo "Distribution détectée: $NAME $VERSION"
        case $ID in
            debian|ubuntu|linuxmint)
                gestionnaire_paquets="apt"
                echo "Gestionnaire de paquets: APT"
                ;;
            fedora|rhel|centos)
                gestionnaire_paquets="dnf"
                echo "Gestionnaire de paquets: DNF"
                ;;
            arch|manjaro)
                gestionnaire_paquets="pacman"
                echo "Gestionnaire de paquets: Pacman"
                ;;
            opensuse*)
                gestionnaire_paquets="zypper"
                echo "Gestionnaire de paquets: Zypper"
                ;;
            *)
                gestionnaire_paquets="apt"
                echo "Gestionnaire de paquets par défaut: APT"
                ;;
        esac
    else
        echo "Distribution non détectée, utilisation d'APT par défaut"
        gestionnaire_paquets="apt"
    fi
}

# Fonction pour désinstaller l'environnement actuel
function desinstaller_environnement_actuel() {
    local env=$1
    
    declare -A paquets_a_desinstaller=(
        ["plasma"]="plasma-desktop plasma-workspace kwin kscreen sddm"
        ["gnome"]="gnome gnome-shell gdm3"
        ["xfce"]="xfce4 xfce4-goodies lightdm"
        ["cinnamon"]="cinnamon cinnamon-session lightdm"
    )
    
    if [ -n "${paquets_a_desinstaller[$env]}" ]; then
        echo "Désinstallation des paquets de $env..."
        
        case $gestionnaire_paquets in
            "apt")
                sudo apt remove --purge -y ${paquets_a_desinstaller[$env]}
                sudo apt autoremove -y
                ;;
            "dnf")
                sudo dnf remove -y ${paquets_a_desinstaller[$env]}
                ;;
            "pacman")
                sudo pacman -Rns --noconfirm ${paquets_a_desinstaller[$env]}
                ;;
            "zypper")
                sudo zypper remove -y ${paquets_a_desinstaller[$env]}
                ;;
        esac
        
        echo "Environnement $env désinstallé."
    else
        echo "Aucun paquet spécifique à désinstaller pour $env"
    fi
}

# Fonction pour installer un environnement
function installer_environnement() {
    local paquet=$1
    local nom=$2
    ...
    case $gestionnaire_paquets in
        "apt")
            ...
            # Installer également le gestionnaire d'affichage recommandé
            case $paquet in
                *plasma*|*kde*) sudo apt install -y sddm ;;
                *gnome*) sudo apt install -y gdm3 ;;
        ...
        "dnf")
            ...
            ;;
        "pacman")
            sudo pacman -S --noconfirm $paquet xorg-server xorg-xinit
            ;;
        "zypper")
            sudo zypper install -y -t pattern $paquet
            ;;
    esac
    
    echo "$nom installé avec succès."
}

# Fonction pour configurer l'environnement par défaut
function configurer_environnement_par_defaut() {
    local paquet=$1
    
    echo "Configuration de l'environnement par défaut..."
    
    # Configuration selon le gestionnaire de display
    ...
    # Configuration pour systemd
    sudo systemctl set-default graphical.target
}

# Fonction pour lister les environnements installés
function lister_environnements_installes() {
    echo -e "\nENVIRONNEMENTS INSTALLÉS:"
    echo "========================"
    
    if command -v ls /usr/share/xsessions/*.desktop 2>/dev/null; then
        echo "Environnements X11 disponibles:"
        ls /usr/share/xsessions/*.desktop 2>/dev/null | xargs basename -a | sed 's/\.desktop//'
    fi
    
    if command -v ls /usr/share/wayland-sessions/*.desktop 2>/dev/null; then
        echo -e "\nEnvironnements Wayland disponibles:"
        ls /usr/share/wayland-sessions/*.desktop 2>/dev/null | xargs basename -a | sed 's/\.desktop//'
    fi
    
    echo -e "\nGestionnaire d'affichage actuel:"
    if [ -f /etc/X11/default-display-manager ]; then
        cat /etc/X11/default-display-manager
    fi
}

# Fonction pour changer le gestionnaire d'affichage
function changer_gestionnaire_affichage() {
    echo -e "\nCHANGEMENT DE GESTIONNAIRE D'AFFICHAGE"
    echo "======================================"
    
    echo "Gestionnaires disponibles:"
    echo "1. SDDM (recommandé pour KDE)"
    echo "2. GDM3 (recommandé pour GNOME)"
    echo "3. LightDM (universel)"
    echo "4. Ly (console)"
    echo "5. Annuler"
    
    read -p "Votre choix [1-5]: " choix_gestionnaire
    
    case $choix_gestionnaire in
        1) gestionnaire="sddm" ;;
        2) gestionnaire="gdm3" ;;
        3) gestionnaire="lightdm" ;;
        4) gestionnaire="ly" ;;
        5) return 0 ;;
        *) echo "Choix invalide"; return 1 ;;
    esac
    
    echo "Installation de $gestionnaire..."
    
    case $gestionnaire_paquets in
        "apt") sudo apt install -y $gestionnaire ;;
        "dnf") sudo dnf install -y $gestionnaire ;;
        "pacman") sudo pacman -S --noconfirm $gestionnaire ;;
        "zypper") sudo zypper install -y $gestionnaire ;;
    esac
    
    # Configurer comme gestionnaire par défaut
    if [ -f /etc/X11/default-display-manager ]; then
        echo "/usr/sbin/$gestionnaire" | sudo tee /etc/X11/default-display-manager >/dev/null
    fi
    
    # Activer le service
    sudo systemctl enable $gestionnaire
    
    echo "Gestionnaire $gestionnaire configuré avec succès."
    echo "Redémarrez pour voir les changements."
}
